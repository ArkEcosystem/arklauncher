...
it('sends the notification to the user', function () {
    ...
    $serverProvider->token->user->notify(new ServerProviderSecureShellKeyRemovalFailed($serverProvider));

    Notification::assertSentTo($serverProvider->token->user, ServerProviderSecureShellKeyRemovalFailed::class);
});
...
it('can determine if notification should be sent to a user', function () {
    ...
    $notification = new ServerProviderSecureShellKeyRemovalFailed($serverProvider);
...
it('should contain the type of the notification', function () {
    ...
    $notification = (new ServerProviderSecureShellKeyRemovalFailed($serverProvider))->toArray();
...
it('should contain the right content', function () {
    ...
    expect($notification['content'])->toBe(trans('notifications.subjects.server_provider_ssh_key_removal_failed', ['serverProvider' => $serverProvider->name]));
});
